...
public class LinkedList<T> implements IList<T> {
    ...
    /**
     * Constructor con un arreglo de elementos como parámetro. Crea la lista
     * con todos los elementos del arreglo agregados, en el
     * mismo orden.
     *
     * @param valores
     */
    public LinkedList(T[] valores) {
        for (T item : valores) {
            this.agregar(item);
        }
        this.cachedIndex = null;
     ...
     * @param valores
     */
    public LinkedList(IList<T> valores) {
     ...
     */
    @Override
    public void agregar(IList<T> valores) {
        for (int i = 0; i < valores.size(); i++) {
            this.agregar(valores.get(i));
        }
    }
    ...
    public boolean set(int index, T valor) {
        ...
        // se transforma el indice
        index = transformarIndex(index);
        if (index < 0 || index >= this.tam) {
            return false;
        }
        // si el indice es el primero
        if (index == 0) {
            this.cabeza.valor = valor;
            this.cachedIndex = 0;
            this.cachedNode = this.cabeza;
            return true;
        }
        // si el indice es el ultimo
        if (index == this.tam - 1) {
        ...
        Nodo<T> nodo;
        int i;
        // se verifica si se usa la cabeza o el cachedNode
        if (this.cachedIndex == null || (index < this.cachedIndex)) {
            nodo = this.cabeza;
            i = 0;
        } else {
    ...
    public T remover(int index) {
        ...
        int i;
        // se verifica si se usa la cabeza o el cachedNode
        if (this.cachedIndex == null || this.cachedIndex == this.tam - 1 || (index < this.cachedIndex - 1)) {
            aux = this.cabeza;
            i = 0;
        } else {
            aux = this.cachedNode;
        ...
        // se llega al elemento anterior
        for (; i < index - 1; i++) {
            aux = aux.siguiente;
        }
        // se elimina pero se guarda el nodo para retornar su valor.
